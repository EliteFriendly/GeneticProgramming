...
public:
	...
	void addNoise(vector<double> vectorY,int power) {

		int ammount = vectorY.size();
		for (int i = 0; i < ammount; i++) {
			if (rand()%2) {
				vectorY[i] = vectorY[i] + double(rand() % power + rand() % 1)/100.0 * vectorY[i];
			}
			else {
				vectorY[i] = vectorY[i] - double(rand() % power + rand() % 1)/100.0 * vectorY[i];
			}
	...
	void addNoise(int power) {

		int ammount = vectorY.size();
		for (int i = 0; i < ammount; i++) {
			if (rand() % 2) {
				vectorY[i] = vectorY[i] + double(rand() % (power +  1)) / 100.0 * vectorY[i];
			}
			else {
				vectorY[i] = vectorY[i] - double(rand() % (power +  1)) / 100.0 * vectorY[i];
			}
